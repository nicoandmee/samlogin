...
async function substituteAccountAlias(credBlock, config) {
  if (config.AccountAliases) {
    return config.AccountAliases.reduce((acc, alias) => {
...
function onBeforeRequestEvent(details, config, logger) {
  ...
  new LibSaml(samlResponseBase64)
    ...
    .map((role) =>
      sts.assumeRole(config, logger, STS, role, samlResponseBase64)
    )
...
async function locateDataPath(appName) {
  ...
  return path.join(
    dataHome,
    dataHome === process.env.HOME ? `.${appName}` : appName
  );
}
...
(async () => {
  ...
  const configPath =
    (await locateConfig(appName, argv)) ||
  ...
  const browser = await puppeteer.launch({
    ...
    headless: false,
    userDataDir: path.join(await locateDataPath(appName), 'Chrome'),
  });
